...
const radio_sources = new WeakMap();

function same_radio_group(a, b) {
    return (
        b?.matches?.('input[type="radio"]') &&
        a.name !== "" &&
        a.name === b.name &&
        a.form === b.form &&
...
// Setting checked also unchecks peers without emitting their change events.
function sync_radio_group(el) {
    if (!el.name) {
        // No name, no native radio group - nothing to sync.
        return;
    }
    // Narrow the candidate set by name via the selector itself, rather than
    // walking every radio in the root, since a root can hold many unrelated
    // radio groups.
    const peers = el
        .getRootNode()
        .querySelectorAll(`input[type="radio"][name="${CSS.escape(el.name)}"]`);
    for (const peer of peers) {
        if (same_radio_group(el, peer)) {
            for (const update of radio_sources.get(peer) || []) {
                update();
            }
        }
    }
...
// Shared, ref-counted "change" listener: one capture-phase listener per
// document resyncs every radio-bound pat-bind element in that document,
// instead of each element registering (and having to keep registered on)
// its own listener. Listening on ``ownerDocument`` - rather than an
// element's ``getRootNode()`` captured at setup time - also means the
// listener stays valid even if the element is initialized while still
// detached (e.g. inside a cloned ``<template>``) and inserted later.
const radio_listener_refs = new WeakMap(); // document -> ref count

function radio_change_listener(event) {
    if (event.target?.matches?.('input[type="radio"]')) {
        sync_radio_group(event.target);
    }
}

function add_radio_listener(doc) {
    const count = radio_listener_refs.get(doc) || 0;
    if (count === 0) {
        doc.addEventListener("change", radio_change_listener, { capture: true });
    }
    radio_listener_refs.set(doc, count + 1);
}

function remove_radio_listener(doc) {
    const count = (radio_listener_refs.get(doc) || 1) - 1;
    if (count <= 0) {
        radio_listener_refs.delete(doc);
        doc.removeEventListener("change", radio_change_listener, { capture: true });
    } else {
        radio_listener_refs.set(doc, count);
    }
}
...
function get_channel(el, key) {
    ...
    if (!channel) {
        channel = { signal: signal(undefined), users: 0 };
...
function resolve_accessor(el, accessor, sanitize) {
    ...
    if (attr_match) {
        ...
        if (use_property) {
            const radio = attr === "checked" && el.matches('input[type="radio"]');
            return {
                read: () => el[attr],
                write: (v) => {
...
function mutation_config(observe) {
    ...
    if (observe.startsWith("attribute:")) {
        return {
            attributes: true,
            attributeFilter: [observe.slice("attribute:".length)],
        };
    }
    return {};
}
...
class Pattern extends BasePattern {
    ...
    async init() {
        ...
        this._listeners = []; // event targets and listener ids, for cleanup
        this._channel_releases = [];
    ...
    setup_binding(binding, sanitize) {
        ...
        const channel = get_channel(this.el, key);
        const sig = channel.signal;
        this._channel_releases.push(channel.release);
    ...
    setup_source(sig, accessor, key) {
        ...
        if (accessor.radio) {
            let sources = radio_sources.get(this.el);
            if (!sources) {
                sources = new Set();
                radio_sources.set(this.el, sources);
            }
            sources.add(handler);
            const doc = this.el.ownerDocument;
            add_radio_listener(doc);
            this._disposers.push(() => {
                sources.delete(handler);
                remove_radio_listener(doc);
            });
        } else if (accessor.event) {
            const id = `pat-bind--${key}--${this.uuid}`;
            events.add_event_listener(this.el, accessor.event, id, handler, {});
            this._listeners.push([this.el, id]);
    ...
    destroy() {
        ...
        for (const [target, id] of this._listeners) {
            events.remove_event_listener(target, id);
        }
        for (const release of this._channel_releases.splice(0)) {
            release();
        }
        super.destroy();
    }
