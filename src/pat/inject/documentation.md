...
| `on-redirect`     | `follow`                                    | `follow`, `reload`                      | Controls what happens when the server redirects the AJAX request to a different URL. `follow` (default) injects content from the redirect target, with all relative URLs in the response correctly rebased against the final URL. `reload` performs a full page navigation to the redirect target URL instead of injecting.                                                                      | One of the allowable values.                 |
