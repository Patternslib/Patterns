...
define(["../../../src/lib/dependshandler.js"], function(DependsHandler) {

    describe("pat-dependshandler", function() {

        beforeEach(function() {
            jq("<div/>", {id: "lab"}).appendTo(document.body);
        });
        ...
        describe("_findInputs", function() {
            ...
            it("Restrict searches to current form", function() {
                ...
                var handler = new DependsHandler(jq("#context"), "foo"),
        ...
        describe("_getValue", function() {
            it("Unchecked checkbox", function() {
                jq("#lab").append("<input type=\"checkbox\" name=\"foo\" value=\"bar\"/>");
                var handler = new DependsHandler(jq("#lab"), "foo");
                expect(handler._getValue("foo")).toBeNull();
            });

            it("Checked checkbox", function() {
                jq("#lab").append("<input type=\"checkbox\" name=\"foo\" value=\"bar\" checked=\"checked\"/>");
                var handler = new DependsHandler(jq("#lab"), "foo");
                expect(handler._getValue("foo")).toBe("bar");
            });

            it("Unchecked radio button", function() {
                jq("#lab").append("<input type=\"radio\" name=\"foo\" value=\"bar\"/>");
                var handler = new DependsHandler(jq("#lab"), "foo");
                expect(handler._getValue("foo")).toBeNull();
            });
            ...
            it("Checked radio button", function() {
                jq("#lab").append("<input type=\"radio\" name=\"foo\" value=\"bar\" checked=\"checked\"/>");
                var handler = new DependsHandler(jq("#lab"), "foo");
                expect(handler._getValue("foo")).toBe("bar");
            });
        });
        ...
        describe("getAllInputs", function() {
            ...
            it("Nested expression", function() {
                ...
                var handler = new DependsHandler(jq("#lab"), "foo or (foo and buz)"),
                    inputs = handler.getAllInputs();
        ...
        describe("evaluate", function() {
            describe("truthy", function() {
                it("Text input with value", function() {
                    jq("#lab").append("<input type=\"text\" name=\"foo\" value=\"xxx\"/>");
                    var handler = new DependsHandler(jq("#lab"), "foo");
                    expect(handler.evaluate()).toBe(true);
                });

                it("Text input without value", function() {
                    jq("#lab").append("<input type=\"text\" name=\"foo\"/>");
                    var handler = new DependsHandler(jq("#lab"), "foo");
            ...
            describe("Comparison", function() {
                it("Positive number input below value", function() {
                    jq("#lab").append("<input type=\"number\" name=\"foo\" value=\"10\"/>");
                    var handler = new DependsHandler(jq("#lab"), "foo<15");
                    expect(handler.evaluate()).toBe(true);
                });

                it("Negative number input below value", function() {
                    jq("#lab").append("<input type=\"number\" name=\"foo\" value=\"20\"/>");
                    var handler = new DependsHandler(jq("#lab"), "foo<15");
                    expect(handler.evaluate()).toBe(false);
                });

                it("Positive equal to value", function() {
                    jq("#lab").append("<input type=\"text\" name=\"foo\" value=\"bar\"/>");
                    var handler = new DependsHandler(jq("#lab"), "foo=bar");
                    expect(handler.evaluate()).toBe(true);
                });

                it("Negative equal to value", function() {
                    jq("#lab").append("<input type=\"text\" name=\"foo\" value=\"buz\"/>");
                    var handler = new DependsHandler(jq("#lab"), "foo=bar");
                    expect(handler.evaluate()).toBe(false);
                });
            });
            ...
            describe("Negate test", function() {
                it("Text input with value", function() {
                    jq("#lab").append("<input type=\"text\" name=\"foo\" value=\"xxx\"/>");
                    var handler = new DependsHandler(jq("#lab"), "not foo");
                    expect(handler.evaluate()).toBe(false);
                });

                it("Text input without value", function() {
                    jq("#lab").append("<input type=\"text\" name=\"foo\" value=\"\"/>");
                    var handler = new DependsHandler(jq("#lab"), "not foo");
                    expect(handler.evaluate()).toBe(true);
                });
            });
            ...
            describe("AND multiple expressions", function() {
                it("All options true", function() {
                    ...
                    var handler = new DependsHandler(jq("#lab"), "one and two");
                    expect(handler.evaluate()).toBe(true);
                });
            ...
            describe("OR multiple expressions", function() {
                it("No options true", function() {
                    ...
                    var handler = new DependsHandler(jq("#lab"), "one or two");
                    expect(handler.evaluate()).toBe(false);
                });
