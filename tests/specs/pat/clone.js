define(["pat-registry", "pat-clone"], function(registry) {
    ...
    describe("pat-clone", function() {
        ...
        it("clones the node's first child when .clone-trigger is clicked and places the clone after the cloned element", function() {
            var $lab = $('#lab');
            $lab.html(
                '<div class="pat-clone">' +
            ...
            $lab.find('.clone-trigger').click();
            expect($('div.item').length).toBe(2);
            $lab.find('.clone-trigger:last').click();
            expect($('div.item').length).toBe(3);
            $lab.find('.clone-trigger:first').click();
        ...
        it("allows the trigger element to be configured", function() {
            var $lab = $('#lab');
            $lab.html(
                '<div class="pat-clone" data-pat-clone="trigger-element: .real-clone-trigger;">' +
                '    <div class="item">Clone Me</div>' +
                '    <button class="clone-trigger">Will not Clone!</button>' +
                '    <button class="real-clone-trigger">Clone!</button>' +
                '</div>');
            registry.scan($lab);
            expect($('div.item').length).toBe(1);
            $lab.find('.clone-trigger').click();
            expect($('div.item').length).toBe(1);
            $lab.find('.real-clone-trigger').click();
            expect($('div.item').length).toBe(2);
        });
