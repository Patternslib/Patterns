        ...
        describe('Date picker with i18n', function() {
            ...
            describe('with proper json URL', function() {
                ...
                it('properly localizes the months and weekdays', function() {
                    var $pika = $('<input type="date" class="pat-date-picker" value="2018-10-21" data-pat-date-picker="i18n: ' + url + '" />').appendTo(document.body);
                    ...
                    expect(
                        document.querySelector('.pika-lendar .pika-select-month option[selected="selected"]').textContent
                    ).toBe('Oktober');
                });
            ...
            describe('with bogus json URL', function() {
                ...
                it('falls back to default (english) month and weekday labels ', function() {
                    var $pika = $('<input type="date" class="pat-date-picker" value="2018-10-21" data-pat-date-picker="i18n: ' + url + '" />').appendTo(document.body);
                    spyOn($, "ajax").and.callThrough().and.returnValue(deferred);
                    pattern.init($pika);
                    deferred.reject();
                    $pika.click();
