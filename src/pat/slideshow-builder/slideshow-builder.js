...
var log = logging.getLogger("slideshow-builder"),
    builder = {
        ...
        onError: function(jqXHR, textStatus, errorThrown) {
            log.error(
                "Error loading slidethis from " + this.action,
                textStatus,
                errorThrown,
                this
            );
        ...
        onLoad: function(html) {
            ...
            if (!$slides.length) {
                log.warn(
                    "Could not find any slides in " + this.action,
                    this
                );
                return;
            }
            ...
            if (!headers.length) {
                log.warn(
                    "No slides with headers found in " + this.action,
                    this
                );
                return;
            }
            ...
            for (i = 0; i < headers.length; i++) {
                ...
                label.appendChild(
                    document.createTextNode(headers[i].title)
                );
                fieldset.appendChild(label);
            }
            this.insertBefore(fieldset, this.firstChild);
            registry.scan(fieldset);
        }
    };
